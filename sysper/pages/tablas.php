...
<?php endforeach; ?>
  ...
  <script>
    // Configura tu API key de OpenWeatherMap aquí
    const API_KEY = '1d6e761cdb5391bbe0968b05fd700d6a';
    const CIUDAD_ID = '3824091'; // ID de Tuxpan, Veracruz
    
    // Función para obtener los datos del clima
    async function obtenerClima() {
        try {
            // Obtener clima actual
            const respuesta = await fetch(`https://api.openweathermap.org/data/2.5/weather?id=${CIUDAD_ID}&appid=${API_KEY}&units=metric&lang=es`);
            const datos = await respuesta.json();
    
            // Obtener pronóstico
            const respuestaPronostico = await fetch(`https://api.openweathermap.org/data/2.5/forecast?id=${CIUDAD_ID}&appid=${API_KEY}&units=metric&lang=es`);
            const datosPronostico = await respuestaPronostico.json();
    
            actualizarUI(datos, datosPronostico);
        } catch (error) {
    ...
    function actualizarUI(datos, datosPronostico) {
        // Actualizar temperatura y detalles principales
        document.getElementById('temperatura-actual').textContent = `${Math.round(datos.main.temp)}°C`;
        document.getElementById('sensacion-termica').textContent = `Sensación térmica: ${Math.round(datos.main.feels_like)}°C`;
        document.getElementById('humedad').textContent = `Humedad: ${datos.main.humidity}%`;
        document.getElementById('viento').textContent = `Viento: ${Math.round(datos.wind.speed * 3.6)} km/h`; // Convertir m/s a km/h
        document.getElementById('presion').textContent = `Presión: ${datos.main.pressure} hPa`;
        document.getElementById('visibilidad').textContent = `Visibilidad: ${(datos.visibility / 1000).toFixed(1)} km`;
    
        // Actualizar icono del clima
        const iconoClima = obtenerIconoClima(datos.weather[0].icon);
        document.getElementById('clima-icon').className = `fas ${iconoClima} text-warning fa-4x mb-3`;
    
        // Actualizar hora de última actualización
        const fecha = new Date();
        document.getElementById('ultima-actualizacion').textContent = `Actualizado: ${fecha.toLocaleTimeString()}`;
    
        // Actualizar pronóstico por horas
        const pronosticoContainer = document.getElementById('pronostico-horas');
        pronosticoContainer.innerHTML = ''; // Limpiar pronóstico anterior
    
        // Mostrar próximas 4 mediciones
        for (let i = 0; i < 4; i++) {
            const pronostico = datosPronostico.list[i];
            const hora = new Date(pronostico.dt * 1000).getHours();
            const temperatura = Math.round(pronostico.main.temp);
            const icono = obtenerIconoClima(pronostico.weather[0].icon);
    
            const elementoPronostico = document.createElement('div');
            elementoPronostico.className = 'text-center';
            elementoPronostico.innerHTML = `
                <p class="mb-1">${hora}:00</p>
                <i class="fas ${icono} text-primary"></i>
                <p class="mb-0">${temperatura}°C</p>
            `;
            pronosticoContainer.appendChild(elementoPronostico);
        }
    }
    
    // Función para mapear códigos de iconos de OpenWeather a Font Awesome
    ...
    obtenerClima();
    setInterval(obtenerClima, 30 * 60 * 1000);
    </script>
  <script>
